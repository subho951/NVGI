...
<?php if($table == 'payment_details'){?>
<button class="export-btn btn btn-sm" data-format="csv" data-table="{{ $table }}"
    data-columns="{{ implode(',', $columns) }}" data-conditions='@json($conditions ?? [])'
    data-titles="{{ implode(',', $headers) }}" data-filename="{{ $filename ?? 'export' }}"
...
<div class="d-flex justify-content-between align-items-center mb-2">
    <div class="table-toolbox-left">
        <label for="{{ $containerId }}-perPage" class="form-label me-2 table-toolbox-label">Show</label>
        <select id="{{ $containerId }}-perPage" class="form-select d-inline-block"
            style="width: 78px !important;">
        ...
        <span class="table-toolbox-label">entries</span>
    </div>
    <input type="text" id="{{ $searchId }}" class="table-toolbox-search" placeholder="Search...">
</div>



<div class="mt-3 table-responsive" id="{{ $containerId }}"></div>
...
<script>
    document.addEventListener('DOMContentLoaded', () => {
        loadTable({
            ...
            submissionStatusColumn: "{{ $submissionStatusColumn ?? '' }}",
            finalSubmitColumn: "{{ $finalSubmitColumn ?? '' }}",

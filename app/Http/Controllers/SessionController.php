...
class SessionController extends Controller
{
    ...
    /* list */
        public function list(Request $request){
            ...
            $data['rows']                   = Session::where('status', '!=', 3)->orderBy('id', 'DESC')->get();
            $data['action']                 = 'Add';
    ...
    /* edit */
        public function edit(Request $request, $id){
            ...
            $page_name                      = 'session.edit';
            $data['single_row']             = Session::where($this->data['primary_key'], '=', $id)->first();
            $generalSetting                 = GeneralSetting::find('1');
            $data['action']                 = 'Edit';

            if($request->isMethod('post')){
                $member = Session::findOrFail($id);

                $request->validate([
                    'name'         => 'required|string|max:255|unique:sessions,name,'.$member->id,
                ]);
    ...
    /* delete */
        public function delete(Request $request, $id){
            ...
            Session::where($this->data['primary_key'], '=', $id)->update($fields);

...
class SubjectController extends Controller
{
    ...
    /* edit */
        public function edit(Request $request, $id){
            ...
            $page_name                      = 'subject.edit';
            $data['single_row']             = Subject::where($this->data['primary_key'], '=', $id)->first();
            $generalSetting                 = GeneralSetting::find('1');
            $data['action']                 = 'Edit';

            if($request->isMethod('post')){
                $member = Subject::findOrFail($id);

                $request->validate([
                    'name'         => 'required|string|max:255|unique:subjects,name,'.$member->id,
                ]);

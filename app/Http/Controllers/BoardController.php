...
class BoardController extends Controller
{
    ...
    /* list */
        public function list(Request $request){
            ...
            $data['rows']                   = Board::where('status', '!=', 3)->orderBy('id', 'DESC')->get();
            $data['action']                 = 'Add';
    ...
    /* edit */
        public function edit(Request $request, $id){
            ...
            $data['single_row']             = Board::where($this->data['primary_key'], '=', $id)->first();
            $generalSetting                 = GeneralSetting::find('1');
            $data['action']                 = 'Edit';

            if($request->isMethod('post')){
                $member = Board::findOrFail($id);

                $request->validate([
                    'name'         => 'required|string|max:255|unique:boards,name,'.$member->id,
                ]);
    ...
    /* delete */
        public function delete(Request $request, $id){
            ...
            Board::where($this->data['primary_key'], '=', $id)->update($fields);
